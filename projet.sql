...
CREATE TABLE Gestionnaire_abonnee(
    nom_gest VARCHAR(50) PRIMARY KEY,
    prenom_gest VARCHAR(50) NOT NULL,
    adres_gest VARCHAR(20) NOT NULL,
    tel_gest INT NOT NULL
);
...
ALTER TABLE Emprunteur
ADD CONSTRAINT fk_votre_table_gestionnaire_abonnee
FOREIGN KEY (nom_gest) REFERENCES gestionnaire_abonnee(nom_gest);
...
-- Insertions dans la table Gestionnaire_abonnee
INSERT INTO gestionnaire_abonnee (nom_gest, prenom_gest, adres_gest, tel_gest) VALUES
('Durand', 'Alice', '123 Rue Exemple', 123456789),
('Martin', 'Bob', '456 Rue Exemple', 987654321),
('Dupont', 'Chloe', '789 Rue Exemple', 234567890),
('Petit', 'David', '101 Rue Exemple', 345678901),
('Lefevre', 'Emma', '202 Rue Exemple', 456789012);
...
INSERT INTO emprunteur (id_emprunt, nom_emprunt, prenom_emprunt, sexe_emprunt, adresse_emprunt, tel_emprunteur, matricul_biblio, id_fich, num_biblio, nom_gest) VALUES
...
(1, 'Bodin', 'Jean-Christophe', '1950-04-05', 'Française'),  -- Auteur de "Le Jour des Corneilles"
...
(36, 'Un secret', 2023, 9782070409358, 'Français', 36, 9, 'https://example.com/download36', 'Un drame familial émouvant qui révèle des secrets longtemps gardés. À travers les souvenirs et les découvertes du protagoniste, le roman explore les thèmes de la mémoire, de l''identité et de la réconciliation avec le passé.');

DROP TABLE IF EXISTS carte_biblio CASCADE;
DROP TABLE IF EXISTS gestionnaire_abonnee CASCADE;
